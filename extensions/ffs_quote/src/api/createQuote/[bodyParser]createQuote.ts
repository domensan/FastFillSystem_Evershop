...
export default async (request, response) => {
  const {
    ...
    country = '', industry = '', message = '', items, requestType
  } = request.body ?? {};
  const isContact = requestType === 'contact';
  const required = isContact ? [name, email, message] : [name, company, email, phone, city, country, industry];
  if (!required.every((value) => typeof value === 'string' && value.trim())
    ...
    return response.status(400).json({ error: isContact
      ...
      : 'Contact, industry, and products are required.' });
  }
  ...
  try {
    await insert('ffs_quote_request').given({
      ...
      industry: isContact ? '' : industry.trim(),
    ...
    const template = `
      ...
      {{#if city}}<p><strong>Location:</strong> {{city}}, {{country}}</p>{{/if}}
    ...
    `;
    const data = { reference, name, company, email, phone, address1, address2, city, country, industry, items: requestItems, message };
    const notifications = [
